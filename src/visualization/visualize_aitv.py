...
if __name__ == "__main__":
    ...
    args.data_dir = Path("data/raw/synth_body" if not args.hand else "data/raw/synth_hand")
    ...
    # Extract different pose parameters.
    ...
    if args.joints:
        # Visualize body joints provided by the paper authors.
        spheres = Spheres(
            ldmks_3d_world,
            name="GT-Joints",
            radius=(0.007 if not args.hand else 0.0013),
        ...
        billboard = Billboard.from_camera_and_distance(
            ...
            rows,
            [img_rgb],
            draw_func(ldmks_2d, 1 if not args.hand else 2, LDMK_CONN["body"] if not args.hand else LDMK_CONN["hand"]),
        )
        ...
        vertex_indices = (
            ...
            if not args.hand
            else np.int64(np.load("src/visualization/vertices/left_hand_vertices.npy"))
        )
        ...
        spheres = Spheres(
            dense_ldmks_3d,
            name="Body_Vertices" if not args.hand else "Hand_Vertices",
            radius=0.005 if not args.hand else 0.001,
            color=(0.0, 0.0, 1.0, 1.0),
        )
        # dense_ldmks_2d = ldmks.project_3d_to_2d(dense_ldmks_3d.squeeze(), camera_to_image, world_to_camera[:3])
        billboard = Billboard.from_camera_and_distance(
            camera,
            5.0,
            cols,
            rows,
            [img_rgb],
            # draw_func(dense_ldmks_2d, 1 if not args.hand else 2, None),
        )
